...
export function generateAgendaPDF(meeting, attendees) {
  return new Promise((resolve) => {
    ...
    topics.forEach((t) => {
      ...
      doc.fontSize(12).text(`  ${timePrefix}[Priority ${t.priority}] ${t.text}${duration}`);
      if (t.notes) doc.fontSize(10).fillColor('#64748b').text(`    Notes: ${t.notes}`).fillColor('black');
      if (t.actionItems?.length) doc.fontSize(10).text(`    Action Items: ${t.actionItems.join(', ')}`);
      doc.moveDown(0.3);
    });
    doc.moveDown();
