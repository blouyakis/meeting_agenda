...
export async function updateMeeting(req, res) {
  const db = getDB();
  const { title, location, startTime, endTime, topics, attendeeIds } = req.body;
  await db.collection('meetings').updateOne(
    { _id: new ObjectId(req.params.id), userId: req.userId },
    { $set: { title, location, startTime, endTime, topics, attendeeIds } }
  );
  res.json({ _id: req.params.id, title, location, startTime, endTime, topics, attendeeIds });
}
