...
import { getMeetings, getMeeting, createMeetingDoc, updateMeeting, deleteMeeting } from '../controllers/meetingController.js';
...
router.get('/:id/pdf', async (req, res) => {
  const db = getDB();
  const meeting = await db.collection('meetings').findOne({ _id: new ObjectId(req.params.id), userId: req.userId });
...
router.post('/:id/email', async (req, res) => {
  ...
  if (!emails.length) return res.status(400).json({ error: 'No attendees have an email address on file.' });
  const topics = (meeting.startTime && meeting.endTime)
    ? allocateTopicTimes(meeting.topics.filter(t => t.text?.trim()), meeting.startTime, meeting.endTime, meeting.steepness ?? 0.5)
    : [...meeting.topics].filter(t => t.text?.trim()).sort((a, b) => a.priority - b.priority);
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to send email. Check your email credentials in .env.' });
  }
});
