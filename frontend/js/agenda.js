...
async function loadAgenda() {
  ...
  const topics = renderTopics(meeting, steepness);
  ...
  document.getElementById('save-actions-btn').addEventListener('click', () =>
    saveActionItems(meeting)
  );
}

function renderTopics(meeting, steepness) {
  const topics = allocateTopicTimes(meeting.topics, meeting.startTime, meeting.endTime, steepness);
...
async function saveActionItems(meeting) {
  const rows = document.querySelectorAll('.agenda-topic-item');
  const updatedTopics = meeting.topics.map((t, idx) => {
    const inputs = document.querySelectorAll(`.action-item-input[data-topic="${idx}"]`);
...
async function sendEmail() {
  const res = await apiFetch(`/api/meetings/${meetingId}/email`, { method: 'POST' });
  if (!res) return;
...
function allocateTopicTimes(topics, startTime, endTime, steepness) {
  ...
  const sorted = [...topics].filter((t) => t.text?.trim()).sort((a, b) => a.priority - b.priority);
