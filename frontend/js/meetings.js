...
function updateActionTrigger(container) {
  const checked = [
    ...container.querySelectorAll(".action-panel input:checked"),
  ].map((cb) => cb.value);
...
function addTopic(text = "", priority = 3, notes = "", actionItems = []) {
  ...
  `;
  div
    .querySelector(".btn-remove-topic")
...
function getFormData() {
  const topics = [...document.querySelectorAll(".topic-row")].map((row) => ({
    ...
    actionItems: [...row.querySelectorAll(".action-panel input:checked")].map(
      (cb) => cb.value,
    ),
  }));
  const attendeeIds = [
